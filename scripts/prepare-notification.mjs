...
const lessons = loadLessons().filter((lesson) => (!selectedLessonId || lesson.id === selectedLessonId) && isPublished(lesson))

if (lessons.length === 0) {
  if (selectedLessonId) {
    throw new Error(`Lesson ${selectedLessonId} was not found, is not available yet, or notification.enabled is not true`)
  }
  console.log('No published lessons with notifications enabled. Nothing to send.')
  process.exit(0)
}
