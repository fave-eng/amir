(() => {
  ...
  async function discoverHomeworkData() {
    ...
    for (let number = 1; number <= maxLessonNumber; number += 1) {
      ...
      if (lesson) {
        if (lesson.status !== 'draft') lessons.push(lesson);
  ...
  const HomeworkReportService = {
    ...
    async send(lessonId, submissionId) {
      ...
      const configuredBase = safeText(config.site?.baseUrl).trim().replace(/\/+$/, '');
      const lessonUrl = configuredBase
        ? `${configuredBase}/lesson.html?id=${encodeURIComponent(lessonId)}`
      ...
      if (result.pending) throw new Error('The report is already being delivered. Try again shortly.');
      return result;
    }
  };
  ...
  function setSyncStatus(state, message) {
    let panel = byId('sync-status');
    if (!panel) {
      const main = document.querySelector('main');
      if (!main) return;
      panel = document.createElement('div');
      panel.id = 'sync-status';
      panel.className = 'sync-status';
  ...
  window.ProgressService = {
    ...
    saveHomeworkProgress(progress) {
      ...
      this.queueSync('homework');
      CloudService.queue('homework');
      setSyncStatus(CloudService.isConfigured() ? 'local' : 'offline', CloudService.isConfigured() ? 'Saved on this device · syncing…' : 'No cloud connection. Changes are saved on this device.');
      return ok;
    },
    ...
    saveGrammarProgress(progress) {
      ...
      this.queueSync('grammar');
      CloudService.queue('grammar');
      setSyncStatus(CloudService.isConfigured() ? 'local' : 'offline', CloudService.isConfigured() ? 'Saved on this device · syncing…' : 'No cloud connection. Changes are saved on this device.');
      return ok;
    },
    async submitHomework(lesson, result, submissionId, submittedAt) {
      if (!CloudService.isConfigured()) throw new Error('Cloud sync is not configured. Final submission requires Supabase.');
      ...
      const { data, error } = await CloudService.client
        .from(tables.homework)
        .upsert(row, { onConflict: 'student_id,lesson_id' })
        .select('*')
        .single();
      if (error) throw error;
      if (!data || data.submission_id !== submissionId || !['submitted_pending_report', 'submitted'].includes(data.status)) {
        throw new Error('Supabase did not confirm the final submitted record.');
      }
      this.clearQueuedSync('homework');
      setSyncStatus('synced', 'Synced');
      return data;
    },
    queueSync(section) {
      const queue = storage.read(key('sync_queue'), []);
      const clean = Array.isArray(queue) ? queue.filter((item) => item && item.section !== section) : [];
      clean.push({ section, queuedAt: new Date().toISOString() });
      storage.write(key('sync_queue'), clean);
      return clean;
    },
    clearQueuedSync(section) {
      const queue = storage.read(key('sync_queue'), []);
      storage.write(key('sync_queue'), (Array.isArray(queue) ? queue : []).filter((item) => item?.section !== section));
    },
    queueVocabularyTest(topicId, outcomes, testResult) {
      const queue = storage.read(key('sync_queue'), []);
      const queueId = safeText(testResult?.completedAt, createUuid());
      const clean = (Array.isArray(queue) ? queue : []).filter((item) => !(item?.section === 'vocabulary-test' && item.queueId === queueId));
      clean.push({ section: 'vocabulary-test', queueId, topicId, outcomes: outcomes || {}, testResult, queuedAt: new Date().toISOString() });
      storage.write(key('sync_queue'), clean);
      return queueId;
    },
    clearQueuedVocabularyTest(queueId) {
      const queue = storage.read(key('sync_queue'), []);
      storage.write(key('sync_queue'), (Array.isArray(queue) ? queue : []).filter((item) => !(item?.section === 'vocabulary-test' && item.queueId === queueId)));
    },
    async flushVocabularyTests() {
      if (!CloudService.isConfigured()) return false;
      if (!CloudService.client) await CloudService.init();
      const queue = storage.read(key('sync_queue'), []);
      const pending = (Array.isArray(queue) ? queue : []).filter((item) => item?.section === 'vocabulary-test');
      for (const item of pending) {
        const { error } = await CloudService.client.rpc('apply_vocabulary_test', {
          p_student_id: studentId,
          p_topic_id: item.topicId,
          p_outcomes: item.outcomes || {},
          p_test: item.testResult || {}
        });
        if (error) throw error;
        this.clearQueuedVocabularyTest(item.queueId);
      }
      return true;
    },
    async saveVocabularyTest(topicId, outcomes, testResult) {
      const progress = this.loadVocabularyProgress();
      Object.entries(outcomes || {}).forEach(([wordKey, outcome]) => {
        const record = VOCABULARY_CATALOG.byKey.get(wordKey);
        if (!record || !['known', 'difficult'].includes(outcome)) return;
        setWordStatus(progress, record.word, topicId, outcome);
      });
      const topicProgress = getTopicProgress(progress, topicId);
      const testKey = safeText(testResult?.completedAt, JSON.stringify(testResult));
      if (!topicProgress.tests.some((test) => safeText(test?.completedAt, JSON.stringify(test)) === testKey)) topicProgress.tests.push(testResult);
      storage.write(key('vocabulary'), normalizeVocabularyProgress(progress));
      const queueId = this.queueVocabularyTest(topicId, outcomes, testResult);
      CloudService.queue('vocabulary');
      if (!CloudService.isConfigured()) return false;
      if (!CloudService.client) await CloudService.init();
      const { error } = await CloudService.client.rpc('apply_vocabulary_test', {
      ...
      if (error) throw error;
      this.clearQueuedVocabularyTest(queueId);
      setSyncStatus('synced', 'Synced');
      return true;
    },
    async syncFromCloud() {
      if (!CloudService.isConfigured()) {
        setSyncStatus('offline', 'No cloud connection. Changes are saved on this device.');
        return false;
      }
      if (!CloudService.client) await CloudService.init();
      ...
      try {
        ...
        (homeworkResponse.data || []).forEach((row) => {
          const cloudFinal = ['submitted_pending_report', 'submitted'].includes(row.status);
          const localResult = homework.results[row.lesson_id];
          const localUpdatedAt = localResult?.updatedAt || localResult?.checkedAt;
          if (cloudFinal || !localResult || dateMs(row.updated_at) >= dateMs(localUpdatedAt)) {
          ...
          if (cloudFinal) {
            homework.submissions[row.lesson_id] = {
              ...
              lockedAt: row.locked_at || row.submitted_at || row.updated_at,
              status: row.status,
              reportStatus: row.report_status || 'not_sent',
        ...
        (grammarResponse.data || []).forEach((row) => {
          ...
          grammar.topics[row.topic_id] = {
            ...
            passedAt: row.passed_at || local.passedAt || null
          };
        });
        ...
        setSyncStatus('offline', 'Cloud sync unavailable. Changes remain on this device.');
        throw error;
      } finally {
      ...
      try {
        if (sections.includes('homework')) {
          ...
          const rows = lessonIds.map((lessonId) => {
            ...
            const total = Number(result.total || 0);
            const correct = Number(result.correct || 0);
            const submitted = Boolean(submission?.submissionId || submission?.savedAt);
            return {
              submission_id: submission?.submissionId || undefined,
              student_id: studentId,
              student_name: safeText(student.nameEn || student.nameRu),
              lesson_id: lessonId,
              lesson_title: safeText(lesson.title, lessonId),
              status: submitted ? (submission.reportStatus === 'sent' ? 'submitted' : 'submitted_pending_report') : (result.checkedAt ? 'checked' : 'draft'),
              answers: result.answers && typeof result.answers === 'object' ? result.answers : {},
              score_correct: result.checkedAt || submitted ? correct : null,
              score_total: result.checkedAt || submitted ? total : null,
              score_percent: result.checkedAt || submitted ? safePercent(correct, total) : null,
              checked_at: result.checkedAt || null,
              submitted_at: submission?.savedAt || null,
              locked_at: submission?.lockedAt || submission?.savedAt || null,
              report_status: submission?.reportStatus || (submitted ? 'pending' : 'not_sent')
            };
        ...
        if (sections.includes('vocabulary')) {
          await this.flushVocabularyTests();
          const progress = this.loadVocabularyProgress();
          ...
          this.clearQueuedSync('vocabulary');
        }
        ...
        if (sections.includes('grammar')) {
          ...
          const rows = Object.entries(progress.topics).map(([topicId, state]) => ({
            ...
            passed_at: state.passedAt || null
          }));
          if (rows.length) {
            const { error } = await client.from(tables.grammar).upsert(rows, { onConflict: 'student_id,topic_id' });
            if (error) throw error;
          }
          this.clearQueuedSync('grammar');
        }
        setSyncStatus('synced', 'Synced');
        return true;
      } catch (error) {
        sections.forEach((name) => this.queueSync(name));
        setSyncStatus('offline', 'Cloud sync unavailable. Changes remain on this device.');
        throw error;
      }
    }
  ...
  function renderReadingSections(block) {
    ...
    return `<div class="reading-sections">${sections.map((section) => `<section class="reading-section">
      ...
      <p class="reading-section-copy">${escapeHtml(section.text || '').replaceAll('\n', '<br>')}</p>
    </section>`).join('')}</div>`;
  }
  ...
  function renderExerciseItem(item, blockId, index) {
    ...
    const media = itemImage?.src
      ? `<figure class="exercise-item-media"><a href="${escapeHtml(itemImage.src)}" target="_blank" rel="noopener"><img src="${escapeHtml(itemImage.src)}" alt="${escapeHtml(itemImage.alt || '')}" loading="lazy"></a></figure>`
      : '';
    return `<div class="exercise-item${media ? ' has-item-media' : ''}" data-exercise-item="${escapeHtml(itemId)}" data-input-type="${escapeHtml(item.input || 'text')}">
      ${media}${itemHeader}
  ...
  function renderLessonBlock(block, index) {
    ...
    if (block.type === 'reorder') {
      ...
      return `<article class="card lesson-block" data-task="${escapeHtml(id)}" data-type="reorder"><h3>${title}</h3><div class="word-chips" data-reorder-source>${chips}</div><div class="button-row compact-actions"><button class="btn btn-ghost" type="button" data-reorder-undo>Undo</button><button class="btn btn-secondary" type="button" data-reorder-clear>Clear</button></div><label class="field-label" for="${escapeHtml(id)}">Your sentence</label><input class="text-field" id="${escapeHtml(id)}" readonly><div class="feedback"></div></article>`;
    }
    if (block.type === 'audio') {
    ...
    console.warn(`Unsupported lesson block type: ${safeText(block.type, 'unknown')}`, block);
    return '';
  }

  function activateMediaFallbacks(scope) {
    scope.querySelectorAll('audio.audio-player').forEach((player) => {
      player.addEventListener('error', () => {
        const message = document.createElement('p');
        message.className = 'muted media-error';
        message.setAttribute('role', 'status');
        message.textContent = 'The audio file is unavailable. Please tell the teacher.';
  ...
  function readExerciseItemValue(item, itemNode) {
    const inputType = item.input || 'text';
    if (inputType === 'example-gap') return itemNode.querySelector('[data-example-gap]')?.value ?? '';
    if (inputType === 'odd-one-out') {
      return {
        selected: itemNode.querySelector('input[type="radio"]:checked')?.value ?? '',
        reason: itemNode.querySelector('[data-odd-reason]')?.value ?? ''
      };
    }
    if (inputType === 'multiple') return [...itemNode.querySelectorAll('input:checked')].map((input) => Number(input.value)).sort((a, b) => a - b);
    if (inputType === 'single') return itemNode.querySelector('input:checked')?.value ?? '';
    if (inputType === 'select') return itemNode.querySelector('select')?.value ?? '';
  ...
  function checkExerciseItem(item, itemNode) {
    const inputType = item.input || 'text';
    const actual = readExerciseItemValue(item, itemNode);
    let correctCount = 0;
    let total = 1;

    if (inputType === 'example-gap') {
      correctCount = textAnswerMatches(item, actual) ? 1 : 0;
    } else if (inputType === 'odd-one-out') {
      const reasonAnswers = Array.isArray(item.reasonAcceptedAnswers) && item.reasonAcceptedAnswers.length
        ? item.reasonAcceptedAnswers
        : [item.reasonAnswer];
      const reasonCorrect = reasonAnswers.some((answer) => normalizeAnswer(answer) && normalizeAnswer(answer) === normalizeAnswer(actual.reason));
      correctCount = actual.selected !== '' && Number(actual.selected) === Number(item.answer) && reasonCorrect ? 1 : 0;
    } else if (inputType === 'multiple') {
      const expected = [...(item.answers || item.answer || [])].map(Number).sort((a, b) => a - b);
      correctCount = JSON.stringify(actual) === JSON.stringify(expected) ? 1 : 0;
    ...
    return { actual, correct: total > 0 && correctCount === total, correctCount, total };
  }
    ...
    (Array.isArray(block.items) ? block.items : []).forEach((item, index) => {
      ...
      if (item.scored === false) {
        ...
        if (feedback) {
          ...
          feedback.textContent = 'Saved for teacher review.';
        }
        return;
      }
      ...
      if (feedback) {
        ...
        feedback.textContent = result.correct
          ? (result.total > 1 ? `Correct: ${result.correctCount} of ${result.total}.` : 'Correct!')
          : hideAnswerOnError
            ? 'Incorrect. Check your answer and try again.'
            : safeText(item.explanation, result.total > 1 ? `${result.correctCount} of ${result.total} gaps are correct.` : 'Check the answer and try again.');
      }
    });
  ...
  function checkLessonTask(block, node) {
    ...
    let actual;
    let correctCount = 0;
    let total = block.scored === false ? 0 : 1;

    if (block.type === 'single') {
      actual = node.querySelector('input:checked')?.value ?? '';
      correctCount = actual !== '' && Number(actual) === Number(block.answer) ? 1 : 0;
    } else if (block.type === 'multiple') {
      actual = [...node.querySelectorAll('input:checked')].map((input) => Number(input.value)).sort((a,b) => a-b);
      const expected = [...(block.answers || block.answer || [])].map(Number).sort((a,b) => a-b);
      correctCount = JSON.stringify(actual) === JSON.stringify(expected) ? 1 : 0;
      ...
      total = Array.isArray(block.pairs) ? block.pairs.length : 0;
      correctCount = actual.reduce((count, value, index) => count + (value === index ? 1 : 0), 0);
    } else {
      actual = node.querySelector('input, textarea')?.value || '';
      const accepted = Array.isArray(block.acceptedAnswers) && block.acceptedAnswers.length
        ? block.acceptedAnswers
        : Array.isArray(block.answer) ? block.answer : [block.answer];
      correctCount = accepted.some((answer) => normalizeAnswer(answer) !== '' && normalizeAnswer(answer) === normalizeAnswer(actual)) ? 1 : 0;
    }
    return { correctCount, total, actual };
  }
    ...
    blocks.forEach((block, blockIndex) => {
      ...
      if (block.type === 'exercise') {
        ...
        (Array.isArray(block.items) ? block.items : []).forEach((item, itemIndex) => {
          ...
          const itemNode = node.querySelector(`[data-exercise-item="${CSS.escape(itemId)}"]`);
          if (itemNode) values[itemId] = readExerciseItemValue(item, itemNode);
        });
        answers[taskId] = values;
      } else if (block.type === 'single') {
        answers[taskId] = node.querySelector('input:checked')?.value ?? '';
      } else if (block.type === 'multiple') {
        answers[taskId] = [...node.querySelectorAll('input:checked')].map((input) => Number(input.value)).sort((a, b) => a - b);
      } else if (block.type === 'select') {
        answers[taskId] = node.querySelector('select')?.value ?? '';
      } else if (block.type === 'match') {
        answers[taskId] = [...node.querySelectorAll('[data-match-index]')].map((select) => select.value);
      } else {
        answers[taskId] = node.querySelector('input, textarea')?.value || '';
      }
    });
    return answers;
  }
  ...
  async function renderLesson() {
    ...
    const lesson = await resolveLessonContent(lessonRecord);
    ...
    const pointsLabel = Number(lesson.totalPoints || 0) > 0 ? `${escapeHtml(lesson.totalPoints)} points` : 'No automatic score';
    const hasManualResponses = blocks.some((block) => block.scored === false || (block.type === 'exercise' && (block.items || []).some((item) => item.scored === false)));
    const lessonSections = blocks.map((block, blockIndex) => block.type === 'section' ? { block, blockIndex } : null).filter(Boolean);
    const roadmap = lessonSections.length >= 2
      ? `<nav class="card lesson-roadmap" aria-label="Lesson roadmap"><div class="lesson-roadmap-heading"><span class="eyebrow">Lesson roadmap</span><p>Complete the sections in order — your answers are saved automatically.</p></div><ol>${lessonSections.map(({ block, blockIndex }, sectionIndex) => `<li><a href="#lesson-section-${blockIndex}"><span>${sectionIndex + 1}</span><strong>${escapeHtml(block.title || `Part ${sectionIndex + 1}`)}</strong></a></li>`).join('')}</ol></nav>`
      : '';
    let sectionNumber = 0;
    ...
    root.innerHTML = `<div class="card lesson-intro"><div><span class="eyebrow">Homework #${Number(lesson.number || 0)}</span><h2>${escapeHtml(lesson.title || 'Homework')}</h2><p>${escapeHtml(lesson.subtitle || 'Complete the tasks below. Your answers are saved automatically.')}</p></div><span class="lesson-points">${pointsLabel}</span></div>
      ${lessonMaterialLinks(lesson, 'lesson')}
      ${roadmap}
      <div id="lesson-blocks">${renderedBlocks}</div>
      <div class="card section lesson-actions"><div id="lesson-result" aria-live="polite"></div><div class="button-row" id="lesson-action-buttons"><button class="btn btn-primary" id="check-lesson" type="button">Check answers</button><button class="btn btn-secondary" id="submit-lesson" type="button" ${savedResult?.checkedAt ? '' : 'disabled'}>Submit to teacher</button></div><p class="muted save-note">Progress saves automatically. Final submission locks the answers.</p></div>`;

    restoreLessonAnswers(root, blocks, savedResult?.answers);
    activateMediaFallbacks(root);

    root.querySelectorAll('[data-reorder-source]').forEach((source) => {
      const parent = source.closest('[data-task]');
      const output = parent?.querySelector('input[readonly]');
      const update = () => {
        if (!output) return;
        output.value = [...source.querySelectorAll('.selected')].map((item) => item.dataset.word).join(' ');
      ...
      parent?.querySelector('[data-reorder-undo]')?.addEventListener('click', () => {
        const selected = [...source.querySelectorAll('.selected')];
        selected.at(-1)?.classList.remove('selected');
        update();
      });
      parent?.querySelector('[data-reorder-clear]')?.addEventListener('click', () => {
        source.querySelectorAll('.selected').forEach((chip) => chip.classList.remove('selected'));
        update();
      });
    });
    ...
    const evaluateLesson = () => {
      ...
      const checkable = blocks.map((block, blockIndex) => ({ block, blockIndex }))
      ...
      checkable.forEach(({ block, blockIndex }) => {
        ...
        if (block.type !== 'exercise') {
          ...
          if (block.scored === false) {
            ...
            const isCorrect = Number(result.total || 0) > 0 && Number(result.correctCount || 0) === Number(result.total || 0);
    ...
    const setReadOnly = (locked) => {
      root.querySelectorAll('#lesson-blocks input, #lesson-blocks textarea, #lesson-blocks select, #lesson-blocks button').forEach((control) => {
        ...
        if (control.matches('input[type="text"], input:not([type]), textarea')) control.readOnly = locked;
      });
      root.querySelectorAll('.exercise-item, [data-task]').forEach((node) => node.classList.toggle('is-locked', locked));
    };

    const reportLabel = (submission) => {
      if (submission?.reportStatus === 'sent') return `Report delivered${submission.reportSentAt ? ` · ${new Date(submission.reportSentAt).toLocaleString()}` : ''}`;
      if (submission?.reportStatus === 'failed') return `Report delivery failed${submission.reportError ? ` · ${escapeHtml(submission.reportError)}` : ''}`;
      return 'Report delivery pending';
    };

    const showSubmittedSummary = (submission, result) => {
      const actionButtons = byId('lesson-action-buttons');
      const score = result || savedResult || { correct: 0, total: 0, percent: 0 };
      byId('lesson-result').innerHTML = `<h3>Homework submitted</h3><p class="muted">Score: ${Number(score.correct || 0)} of ${Number(score.total || 0)} · ${Number(score.percent || 0)}%</p><p class="muted">Submitted: ${submission?.savedAt ? new Date(submission.savedAt).toLocaleString() : 'saved'} · ${reportLabel(submission)}</p>`;
      actionButtons.innerHTML = submission?.reportStatus === 'failed' || submission?.reportStatus === 'pending'
        ? '<button class="btn btn-secondary" id="retry-report" type="button">Retry report delivery</button>'
        : '';
      setReadOnly(true);
      const retry = byId('retry-report');
      if (retry) retry.addEventListener('click', async () => {
        retry.disabled = true;
        retry.textContent = 'Sending…';
        try {
          const response = await HomeworkReportService.send(lesson.id, submission.submissionId);
          const latest = window.ProgressService.loadHomeworkProgress();
          latest.submissions[lesson.id] = { ...latest.submissions[lesson.id], reportStatus: 'sent', reportSentAt: response.sentAt || new Date().toISOString(), reportError: '' };
          window.ProgressService.saveHomeworkProgress(latest);
          showSubmittedSummary(latest.submissions[lesson.id], latest.results[lesson.id]);
        } catch (error) {
          const latest = window.ProgressService.loadHomeworkProgress();
          latest.submissions[lesson.id] = { ...latest.submissions[lesson.id], reportStatus: 'failed', reportError: safeText(error?.message, 'Unknown error') };
          window.ProgressService.saveHomeworkProgress(latest);
          showToast(`Report delivery failed: ${safeText(error?.message, 'unknown error')}`);
          showSubmittedSummary(latest.submissions[lesson.id], latest.results[lesson.id]);
        }
      });
    };

    if (savedResult?.checkedAt) {
      evaluateLesson();
    ...
    if (savedSubmission?.submissionId || savedSubmission?.savedAt) {
      showSubmittedSummary(savedSubmission, savedResult);
      return;
    }

    let draftTimer = 0;
    const saveDraft = (event) => {
      const changedTask = event?.target?.closest?.('[data-task]');
      if (changedTask) {
        const changedItem = event?.target?.closest?.('.exercise-item');
        const stateRoot = changedItem || changedTask;
        stateRoot.classList.remove('is-correct', 'is-wrong', 'is-saved');
        stateRoot.querySelectorAll('.feedback').forEach((feedback) => { feedback.className = 'feedback'; feedback.textContent = ''; });
      }
      window.clearTimeout(draftTimer);
      draftTimer = window.setTimeout(() => {
        const latest = window.ProgressService.loadHomeworkProgress();
        const previous = latest.results[lesson.id] || {};
        latest.results[lesson.id] = {
          ...previous,
          answers: collectLessonAnswers(root, blocks),
          checkedAt: null,
          updatedAt: new Date().toISOString(),
          passedAt: previous.passedAt || (percent === 100 ? new Date().toISOString() : null)
        };
        window.ProgressService.saveHomeworkProgress(latest);
        byId('submit-lesson').disabled = true;
        byId('lesson-result').innerHTML = '<p class="muted">Answers changed. Check them again before submitting.</p>';
      }, 250);
    };
    root.querySelector('#lesson-blocks')?.addEventListener('input', saveDraft);
    root.querySelector('#lesson-blocks')?.addEventListener('change', saveDraft);

    byId('check-lesson').addEventListener('click', () => {
      window.clearTimeout(draftTimer);
      const result = evaluateLesson();
      const manualNote = hasManualResponses ? ' · extended answers are saved for teacher review and are not included in the score' : '';
      ...
      updatedProgress.results[lesson.id] = { ...result, checkedAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    ...
    byId('submit-lesson').addEventListener('click', async () => {
      ...
      if (!window.confirm('After submitting, you will not be able to change your answers. Submit this homework?')) return;
      const result = evaluateLesson();
      const submissionId = createUuid();
      const submittedAt = new Date().toISOString();
      const updatedProgress = window.ProgressService.loadHomeworkProgress();
      updatedProgress.results[lesson.id] = { ...result, checkedAt: submittedAt, updatedAt: submittedAt };
      window.ProgressService.saveHomeworkProgress(updatedProgress);

      button.disabled = true;
      button.textContent = 'Submitting…';
      try {
        const cloudRow = await window.ProgressService.submitHomework(lesson, result, submissionId, submittedAt);
        const confirmed = window.ProgressService.loadHomeworkProgress();
        confirmed.results[lesson.id] = {
          ...result,
          checkedAt: cloudRow.checked_at || submittedAt,
          updatedAt: cloudRow.updated_at || submittedAt,
          cloudStatus: cloudRow.status
        };
        confirmed.submissions[lesson.id] = {
          submissionId: cloudRow.submission_id,
          savedAt: cloudRow.submitted_at || submittedAt,
          lockedAt: cloudRow.locked_at || submittedAt,
          status: cloudRow.status,
          reportStatus: cloudRow.report_status || 'pending',
          reportSentAt: cloudRow.report_sent_at || null,
          reportError: cloudRow.report_error || ''
        };
        if (!confirmed.completedIds.includes(lesson.id)) confirmed.completedIds.push(lesson.id);
        storage.write(key('homework'), confirmed);
        setReadOnly(true);
        try {
          const response = await HomeworkReportService.send(lesson.id, submissionId);
          const latest = window.ProgressService.loadHomeworkProgress();
          latest.submissions[lesson.id] = { ...latest.submissions[lesson.id], status: 'submitted', reportStatus: 'sent', reportSentAt: response.sentAt || new Date().toISOString(), reportError: '' };
          storage.write(key('homework'), latest);
          showToast('Homework submitted. The report was delivered.');
          showSubmittedSummary(latest.submissions[lesson.id], latest.results[lesson.id]);
        } catch (reportError) {
          const latest = window.ProgressService.loadHomeworkProgress();
          latest.submissions[lesson.id] = { ...latest.submissions[lesson.id], reportStatus: 'failed', reportError: safeText(reportError?.message, 'Unknown error') };
          storage.write(key('homework'), latest);
          showToast('Homework is submitted, but the report delivery failed.');
          showSubmittedSummary(latest.submissions[lesson.id], latest.results[lesson.id]);
        }
      } catch (error) {
        ...
        try {
          ...
          const cloudProgress = window.ProgressService.loadHomeworkProgress();
          const cloudSubmission = cloudProgress.submissions[lesson.id];
          if (cloudSubmission?.submissionId) {
            showSubmittedSummary(cloudSubmission, cloudProgress.results[lesson.id]);
            showToast('The final cloud submission was restored.');
            return;
          }
        } catch (syncError) {
          console.warn('Could not verify a final cloud submission:', syncError);
        }
        const latest = window.ProgressService.loadHomeworkProgress();
        delete latest.submissions[lesson.id];
        latest.completedIds = latest.completedIds.filter((lessonId) => lessonId !== lesson.id);
        storage.write(key('homework'), latest);
        setReadOnly(false);
        button.disabled = false;
        button.textContent = 'Submit to teacher';
        showToast(`Submission was not locked: ${safeText(error?.message, 'cloud save failed')}`);
      }
    });
  }
  ...
  const SpeechService = {
    ...
    speak(text, button) {
      if (!this.supported) {
        showToast('Pronunciation is not supported in this browser.');
        return;
      }
      ...
      utterance.rate = 0.85;
      const voice = this.voices.find((item) => /^en-GB/i.test(item.lang)) || this.voices.find((item) => /^en/i.test(item.lang));
  ...
  if (SpeechService.supported) window.speechSynthesis.addEventListener?.('voiceschanged', () => SpeechService.refreshVoices());

  function pronounceButton(word, compact = false) {
    if (!config.features?.wordPronunciation) return '';
    const disabled = SpeechService.supported ? '' : ' disabled';
    return `<button class="pronounce-btn${compact ? ' compact' : ''}" type="button" data-pronounce="${escapeHtml(word.en)}" aria-label="Pronounce ${escapeHtml(word.en)}"${disabled}>🔊</button>`;
  }
  ...
  function renderVocabulary() {
    ...
    modeRoot.addEventListener('click', (event) => {
      ...
      SpeechService.speak(button.dataset.pronounce, button);
    });
    ...
    const resetCardQueue = () => {
      cardQueue = shuffled(topic.words.filter((word) => {
        ...
        return mode === 'difficult'
          ? status === 'difficult'
          : !['known', 'difficult'].includes(status) && !reviewedThisSession.has(word.__wordKey);
      }));
    };
    ...
    const drawCard = () => {
      ...
      modeRoot.innerHTML = `<div class="flash-counter">Remaining: ${remaining}</div><div class="flashcard-stage"><div class="flashcard" id="flashcard" tabindex="0" role="button" aria-label="Flip the card">
        <div class="flash-face flash-front"><div class="flash-word">${escapeHtml(word.en)}</div>${word.transcription ? `<div class="flash-transcription">${escapeHtml(word.transcription)}</div>` : ''}${pronounceButton(word)}<p class="muted">Tap to see the translation</p></div>
      ...
      byId('word-known').addEventListener('click', () => {
        reviewedThisSession.add(word.__wordKey);
        cardQueue.shift();
        drawCard();
      });
    ...
    const startTest = () => {
      ...
      testState = { words: shuffled(topic.words), index: 0, firstTryCorrect: 0, answered: false, firstAnswers: {}, outcomes: {} };
      drawQuestion();
    };
    ...
    const finishTest = () => {
      ...
      Object.entries(testState.outcomes).forEach(([wordKey, status]) => {
        const record = VOCABULARY_CATALOG.byKey.get(wordKey);
        if (record) setWordStatus(progress, record.word, topic.id, status);
      });
      topicProgress.tests.push(result);
      window.ProgressService.saveVocabularyTest(topic.id, testState.outcomes, result).catch((error) => {
        console.error('Vocabulary test transaction failed:', error);
        showToast('The result is saved locally and will sync later.');
      });
      modeRoot.innerHTML = `<div class="card empty-state"><div class="empty-state-icon">🏁</div><h3>Test complete</h3><p>First-try score: ${result.score} of ${result.total}</p><div class="button-row button-row-centered"><button class="btn btn-primary" id="restart-vocab-test" type="button">Try again</button></div></div>`;
    ...
    const drawQuestion = () => {
      ...
      modeRoot.querySelectorAll('[data-answer-key]').forEach((button) => {
        button.addEventListener('click', () => {
          ...
          testState.outcomes[word.__wordKey] = correct ? 'known' : 'difficult';
    ...
    const drawAllWords = () => {
      modeRoot.innerHTML = `<div class="words-grid">${topic.words.map((word) => {
        const status = progress.words[word.__wordKey]?.status;
        return `<article class="card word-card ${status === 'known' ? 'known' : ''} ${status === 'difficult' ? 'difficult' : ''}">${pronounceButton(word, true)}<strong>${escapeHtml(word.en)}</strong><span>${escapeHtml(word.ru)}</span>${word.transcription ? `<span>${escapeHtml(word.transcription)}</span>` : ''}</article>`;
      }).join('')}</div>`;
    };
  ...
  async function init() {
    ...
    window.addEventListener('online', () => {
      window.ProgressService.syncFromCloud().then(refreshCurrentView).catch((error) => console.warn('Queued sync retry failed:', error));
    });
    if (!CloudService.isConfigured()) {
      setSyncStatus('offline', 'No cloud connection. Changes are saved on this device.');
      return;
    }
    setSyncStatus('local', 'Progress saves automatically');
    try {
